...
function publicForwardRequest(request: Request, env: GatewayConfig): Request {
...
function bridgeStreamCancellation(response: Response, cancelLease?: (leaseId: string) => Promise<void>): Response {
  ...
  const body = new ReadableStream<Uint8Array>({
    ...
    async cancel(reason) {
      ...
      if (cancelLease) await cancelLease(leaseId);
    }
  });
...
export async function handleGatewayRequest(
  ...
  context: GatewayRequestContext = {}
    ...
    try {
      const internalGroupControl = handlers.allowInternalControl === true && url.origin === INTERNAL_CONTROL_ORIGIN && INTERNAL_GROUP_PATH.test(url.pathname);
    ...
    handlers.log?.({ requestId, path: url.pathname, method: request.method, status: response.status, durationMs: Date.now() - startedAt });
    return response;
}
