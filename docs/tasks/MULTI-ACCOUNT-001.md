...
实施、必要验证和fresh独立审查完成；发布准备中。MA-01/MA-02关闭，无剩余可执行问题；独立最终14/14通过。用户已明确授权推送及部署v0.2.0-dev.6，下一步核对线上基线、构建打包、限定更新现有flareapi并回读保留原账号/配置；真实多个账号首次授权仍需用户操作官方页面。

保留此前docs/maintenance/batch.md和LOCAL-CLEANUP-001.md未提交清理记录，不纳入多账号成果。真实多个账号授权/调用与线上升级尚未执行，本地模拟通过不代替生产验收。
## 发布准备与授权
