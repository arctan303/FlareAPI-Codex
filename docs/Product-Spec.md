...
版本：多账号本地增量（待发布）；日期：2026-09-18。原版 LIVE-001 已通过；Phase-01 本地扩展已完成，真实两协议/显式思考程度/官方额度/后台操作通过，fresh R2 独立复核通过。证据见 PHASE-01；Phase-02已部署oneapi/api.arcinks.com，管理端可用；云端重新授权后模型目录/额度仍403，端到端验收受阻。
...
## DEC-016 / REQ-24、AC-24：多个账号与默认账号（本地实现及独立审查完成，待发布）
...
兼容/回滚：沿用credentials活跃槽位、加密密钥，新增索引；旧程序只能读取默认槽位，不能在旧程序写入后直接重升级而宣称全量兼容。备份完整存储并恢复同版本，不删除新增账号。任务见[MULTI-ACCOUNT-001](tasks/MULTI-ACCOUNT-001.md)。用户本地完成后明确回复“嗯”，授权v0.2.0-dev.6推送及部署现有flareapi；发布准备中，线上回读另记录。
