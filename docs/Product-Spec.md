...
版本：v0.2.0-dev.6 多账号已发布；日期：2026-09-18。原版 LIVE-001 已通过；Phase-01 本地扩展已完成，真实两协议/显式思考程度/官方额度/后台操作通过，fresh R2 独立复核通过。证据见 PHASE-01；Phase-02已部署oneapi/api.arcinks.com，管理端可用；云端重新授权后模型目录/额度仍403，端到端验收受阻。

## 目标与有效决定

每个部署服务于其账户所有者；支持其他使用者独立安装自己的实例。通过官方页面授权自己的Codex账户，在自己的网站或工具中使用OpenAI风格HTTP接口。当前主方向为轻量单服务器部署：一个Node.js进程和SQLite持久化，Docker可选，生产不依赖Worker/Wrangler/Miniflare；Cloudflare Access管理登录和管理员口令兜底继续保留。DEC-014替代原单Worker最终目标；Worker参考项目核查保留为独立研究，现有实验部署不删除。

| ID | 当前有效决定 | 来源与状态 |
| --- | --- | --- |
| DEC-001 | 单账号范围由 DEC-016 多个自有账号替代；Codex订阅登录仍有效 | 原决定历史保留 |
...
当前支持保存多个自有 Codex 账号并手动选择默认账号，提供文本及函数工具调用接口；原首版单账号 Demo 的历史验收保留。以生成请求为中心，不构建完整 Codex 代理运行环境。

支持保存多个自有账号并手动切换默认账号；不含自动账号池调度、配额规避、转售计费、多人共享、注册系统、支付、自动模型降级、Anthropic/Gemini 格式、WebSocket、图片/音视频、文件上传、托管工具、代码执行、聊天历史数据库和自动云部署。以后需要时另行修订。
...
1. 开发者启动服务，默认监听 loopback 地址，可显式配置私网或 HTTPS 入口；输入管理员口令登录一次，登录后可使用全部后台功能，会话有效期内刷新或重新打开不重复输入。
...
7. “退出后台”只撤销当前管理员会话，不影响 Codex 连接或 API 密钥。旧“断开 Codex”接口清除默认账号与待处理授权、取消上游请求，其他保存账号保留；新后台单账号移除要求空闲，不宣称撤销 OpenAI 全局会话。
...
| REQ-07 / AC-07 | 原生 Node 单服务器交付，保留历史 Worker 适配 | 文档约定的初始化、启动、类型检查、测试、打包命令可运行；本地模拟与实际订阅请求证据分别记录 |
...
## DEC-015 / REQ-18、AC-18：显式私网访问与命令行配置

有效决定（2026-09-07）：用户先选择命令行配置向导，随后补充以启动参数为主要快捷入口，向导保留用于保存默认配置。默认回环保持不变，补充显式 LAN_ORIGINS 私网 origin 和实际 socket peer 校验；不把监听 0.0.0.0 解释为任何 Host 都可信。向导仅更新四个网络字段，保留账号、密钥和 DATA_DIR；有效输入、原子失败保护及拒绝伪造来源需行为验证。完整约束见 [CONSOLE-NET-001](tasks/CONSOLE-NET-001.md)。这替代原仅回环或公网域名可访问的限制，保留 SERVER-001 其他契约。

## REQ-19 / AC-19：可导航的管理后台
...
## DEC-016 / REQ-24、AC-24：多个账号与默认账号（实现、验证、独立审查及发布完成）

用户2026-09-18确认保存多个账号、手动切换默认账号，避免重复授权。替代DEC-001单账号限制；不改变单管理员和API密钥权限。账号各自官方设备授权，加密保存；添加保留原账号，首个成为默认，后续添加不自动切换，相同账号重新授权更新已有记录。列表展示身份提示、套餐、默认标识和重新授权状态，支持切换和移除单个账号，最多32个。

所有API密钥、两个生成接口和模型目录使用默认账号；不增加每key绑定、轮询或自动故障切换。切换和移除要求请求及刷新空闲，忙时明确冲突，不中断生成、不重试到另一账号；待完成设备授权先取消。后台切换清除测试对话、模型和额度旧状态。移除默认后须显式选择其他账号。旧POST /admin/disconnect作为兼容例外，仍立即取消当前请求及设备授权，只移除默认、不清空其他账号；新版后台移除使用空闲保护的DELETE接口。退出管理员不影响账号。

AC-24：旧单账号无损登记为默认，重启保持；添加第二账号不覆盖或切换首个，重新授权不重复；同一现有key切换前后实际上游身份正确、模型与额度不串用；活跃请求及刷新期间切换/移除失败且原请求继续；轮换凭据保存，失败仅停用该账号；非管理员、跨站写入拒绝，列表和错误不含token。

兼容/回滚：沿用credentials活跃槽位、加密密钥，新增索引；旧程序只能读取默认槽位，不能在旧程序写入后直接重升级而宣称全量兼容。备份完整存储并恢复同版本，不删除新增账号。任务见[MULTI-ACCOUNT-001](tasks/MULTI-ACCOUNT-001.md)。用户本地完成后明确回复“嗯”，授权v0.2.0-dev.6推送及部署现有flareapi；已上线并回读原账号/设置、权限、资产与其他资源均通过；真实新增账号仍须各自官方授权，证据见任务。
